...
class ViewMyProfileViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        NotificationCenter.default.addObserver(self, selector: #selector(notifyNewPost), name: NSNotification.Name(rawValue: "NEW_POST_ADDED"), object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(userLoggedIn), name: NSNotification.Name(rawValue: "USER_LOGGED_IN"), object: nil)
        ...
        let settingButton = UIBarButtonItem(image: UIImage(named: "Settings.png"), style: .plain, target: self, action: #selector(goToSettings))
    ...
    func notifyNewPost(notification: NSNotification) {
        let newPost = notification.userInfo?["newPost"] as! Post
    ...
    func userLoggedIn(notification: NSNotification) {
        Types.tryblock({ 
            if let justLoggedUser = notification.userInfo?["loggedUser"] as? BackendlessUser {
                print("user just logged in: \(justLoggedUser.objectId!)")
                self.updateUserIfNeeded(justLoggedUser)
            } else {
                // log in again , error 
                self.handleLogout()
            }
            ...
            print("Backendless server error \(exception)")
        })
    }
